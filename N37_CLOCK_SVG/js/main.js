...
function btnActive(eo) {
  ...
  if (100 < widhtUser.value && widhtUser.value < 2000) {
    ...
    widhtUser.value = prompt("please enter width clock");
    btnActive(eo);
  }
}
...
function clockSVG() {
  ...
  // create face clock
  const clockCircle = document.createElementNS(
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    // create 12 circle
    const clockNum = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
    ...
    clockNum.setAttribute(
      "cx",
      CLOCK_CENTER_X + (CLOCK_RADIUS - CLOCK_WIDTH / 10) * Math.sin(angle)
    );
    clockNum.setAttribute(
      "cy",
      CLOCK_CENTER_Y - (CLOCK_RADIUS - CLOCK_WIDTH / 10) * Math.cos(angle)
    );
  ...
  // create hours hand
  const handHour = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "line"
  );
  ...
  // create minute hand
  const handMin = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "line"
  );
  ...
  // create seconds hand
  const handSec = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "line"
  );
